...
# DAsh: Pi-mono Super Agent routing + LangGraph Sub Agent prototype v8.
#
...
# - v8은 단일 조회/문의 버튼에서 입력 의도에 따라 표준용어 Vector DB 조회 또는 문의답변 RAG+LLM 답변을 자동 선택합니다.
#
# 실행 방법:
#     streamlit run 05.10_prompt_v8.py
#     python 05.10_prompt_v8.py "표준용어 등록 절차가 뭐야?"
#     python 05.10_prompt_v8.py --docs
...
class ProjectDocuments:
    ...
    def _built_in_faq(self) -> list[AnswerDocument]:
        ...
        faqs = [
            ...
            (
                "문의 답변 RAG 처리 절차",
                "v8은 표준용어 설명 검색 Vector DB와 별개로 문의답변 Vector DB를 구성합니다. 조회/문의 버튼을 누르면 사용자 입력 의도에 따라 표준용어 Vector DB 조회 또는 문의답변 RAG+LLM 답변을 자동 수행합니다.",
                "faq",
...
def run_cli(args: list[str]) -> int:
    ...
    if "--docs" in args:
        ...
        print("\n실행 방법: streamlit run 05.10_prompt_v8.py")
        return 0
