...
# DAsh: Pi-mono Super Agent + LangGraph Sub Agent prototype.
#
# 사용자가 전달한 Pi-mono 예시의 핵심 원칙을 Python으로 반영했습니다.
# - 내부 대화는 AgentMessage로 유지하고 LLM 호출 경계에서만 provider message로 변환합니다.
# - Super Agent는 Pi-mono 스타일 event stream(SSE 로그)을 만들고 turn/memory를 관리합니다.
# - Sub Agent는 LangGraph node/edge(router/select/recommend/validate/change)로 구성합니다.
# - Mock RDB/Vector DB로 바로 실행되며, .env 설정으로 Oracle/FAISS/OpenAI 교체 지점을 둡니다.
#
# 실행 방법:
#     streamlit run 05.07_prompt_v2.py
#     python 05.07_prompt_v2.py "작업이 시작된 일시"
...
class RouterDecision(BaseModel):
    """router_agent가 반드시 JSON으로 반환하는 분류 결과입니다."""
...
class DashGraph:
    ...
    def _build_graph(self) -> Any:
        ...
        graph.add_node("router_agent", self._wrap(self.router_agent))
        ...
        graph.add_conditional_edges("router_agent", self._route, {"select_agent": "select_agent", "recommend_agent": "recommend_agent", "validate_agent": "validate_agent", "finalize_agent": "finalize_agent"})
    ...
    def _route(self, raw: dict[str, Any]) -> str:
        """router_agent 이후 intent에 따라 다음 노드를 선택합니다."""
    ...
    def router_agent(self, state: AgentTurnState) -> AgentTurnState:
        """사용자 입력을 intent JSON으로 분류합니다."""

        try:
            state.router_decision = self.router_skill.classify(state.user_input, state.lookup_count)
            payload = state.router_decision.model_dump(mode="json")
            state.logs.append(event("message_end", "router_agent", "intent JSON 반환", json=payload))
            state.messages.append(AgentMessage(role="toolResult", name="router_agent", content=json.dumps(payload, ensure_ascii=False)))
        except Exception as exc:
            state.error = f"router_agent 오류: {exc}"
        return state
...
def css() -> None:
    """전문 블로그 형태의 Streamlit 스타일을 적용합니다."""

    st.markdown("""
    <style>
    header[data-testid="stHeader"]{height:0;background:transparent;visibility:hidden}
    .stApp{background:#f7f5f0;color:#1d252c}.block-container{max-width:1180px;padding-top:.25rem}
    .dash-title{font-family:Georgia,'Times New Roman',serif;font-size:4.2rem;line-height:1;color:#13251f;margin-top:0;margin-bottom:.4rem}
    .dash-lead{font-size:1.05rem;line-height:1.75;color:#47534f;border-left:4px solid #2f6f5e;padding-left:1rem;margin-bottom:2rem}
    .term-card{background:#fffdfa;border:1px solid #ddd5c8;border-radius:8px;padding:1rem;margin-bottom:.75rem}
    .agent-log{background:#13251f;color:#e8efe9;border-radius:8px;padding:1rem;font-family:Consolas,monospace;font-size:.86rem;max-height:360px;overflow-y:auto;white-space:pre-wrap}
    div.stButton>button{border-radius:6px;border:1px solid #2f6f5e}
    </style>
...
def render_app() -> None:
    ...
    st.markdown('<div class="dash-lead">용어 설명을 입력하면 Vector DB에서 기존 표준용어를 조회합니다. 1회 이상 조회한 뒤 추천을 눌러 후보를 만들고, 신청/수정/삭제는 검증 후 RDB와 Vector DB에 반영합니다.</div>', unsafe_allow_html=True)
    left, right = st.columns([1.05, .95], gap="large")
    with left:
        st.subheader("사용자 입력")
        user_text = st.text_area("입력", height=180, placeholder="작업이 시작된 일시\n'작업시작일시'의 설명을 '작업이 시작된 일시'로 변경해줘", label_visibility="collapsed")
        if st.button("전송", use_container_width=True) and user_text.strip():
            execute_prompt(runtime, user_text.strip())
        st.subheader("처리 결과")
        st.write(st.session_state["answer"] or "아직 처리 결과가 없습니다.")
    with right:
        st.subheader("추천 용어")
        if st.button("추천", disabled=st.session_state["lookup_count"] < 1, use_container_width=True):
            execute_prompt(runtime, "추천: " + (st.session_state["last_description"] or ""))
        for i, rec in enumerate(cast(list[Recommendation], st.session_state["recs"])):
            st.markdown(f'<div class="term-card"><b>{rec.logical_name}</b><br>{rec.reason}<br><small>score={rec.score:.2f}</small></div>', unsafe_allow_html=True)
            if st.button(f"{rec.logical_name} 신청", key=f"apply_{i}", use_container_width=True):
                execute_prompt(runtime, f"'{rec.logical_name}' 신청: {rec.description}")
        st.subheader("유사 표준용어")
        for item in cast(list[SimilarTerm], st.session_state["similar"]):
            st.markdown(f'<div class="term-card"><b>{item.term.logical_name}</b><br>{item.term.description}<br><small>similarity={item.similarity:.2f}</small></div>', unsafe_allow_html=True)
    st.subheader("Agent 처리 로그")
    st.markdown(f'<div class="agent-log">{"<br><br>".join(cast(list[str], st.session_state["events"])) or "SSE 이벤트가 여기에 표시됩니다."}</div>', unsafe_allow_html=True)
...
def run_cli(args: list[str]) -> int:
    ...
    for line in stream.sse():
        print(line)
    print("\n실행 방법: streamlit run 05.07_prompt_v2.py")
    return 0
