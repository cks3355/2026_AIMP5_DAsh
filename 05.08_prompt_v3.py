...
def css() -> None:
    ...
    .block-container{max-width:1360px;padding-top:.4rem;padding-bottom:1rem}
    ...
    .dash-lead{font-size:.92rem;line-height:1.6;color:#56616d;border-left:3px solid #337566;padding-left:.8rem;margin-bottom:1.1rem}
...
def render_recommendations(runtime: Runtime) -> None:
    ...
    for i, rec in enumerate(recommendations):
        ...
        if checked and selected_idx != i:
            ...
            selected_idx = None
        st.markdown(
            f'<div class="term-card">{rec.reason}<br><small>score={rec.score:.2f}</small></div>',
            unsafe_allow_html=True,
        )
...
def render_app() -> None:
    ...
    st.markdown('<div class="dash-lead">좌측 2/3 영역에서 입력, 조회, 추천, 등록, 변경을 처리하고 우측 1/3 영역에서 처리 상태를 확인합니다.</div>', unsafe_allow_html=True)

    work_area, status_area = st.columns([2, 1], gap="large")
    with work_area:
        ...
        lookup_result_col, recommend_result_col = st.columns([1, 1], gap="large")
...
def run_cli(args: list[str]) -> int:
    ...
    for line in stream.sse():
        print(line)
    print("\n실행 방법: streamlit run 05.08_prompt_v3.py")
    return 0
