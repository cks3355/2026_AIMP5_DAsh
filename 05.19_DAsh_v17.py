...
def css() -> None:
    ...
    @media (max-width: 900px){
        .dash-topbar,.dash-lead{width:100%}
    }
    .panel-title{
        font-family:"Times New Roman","Malgun Gothic",Times,serif;
    ...
    .chat-panel{
        background:#ffffff;border:1px solid #dfe4ec;border-radius:8px;
        box-shadow:0 1px 3px rgba(16,24,40,.05);
        height:520px;overflow-y:auto;padding:.65rem .72rem;
        box-sizing:border-box
    }
    .chat-empty{
        color:#86868b;font-size:.84rem;line-height:1.45
    }
    .workflow-turn{
        border-bottom:1px solid #edf0f5;margin:0 0 .7rem;padding:0 0 .65rem
    }
    .workflow-turn:last-child{border-bottom:0;margin-bottom:0}
    .workflow-step{
        display:flex;flex-direction:column;margin:0 0 .45rem
    }
    .workflow-step.user{align-items:flex-start}
    .workflow-step.agent{align-items:flex-end}
    .workflow-step .chat-label{
        max-width:92%
    }
    .workflow-action{
        max-width:92%;border-radius:12px;padding:.44rem .58rem;
        font-size:.8rem;line-height:1.4;white-space:pre-wrap;word-break:break-word;
        box-sizing:border-box
    }
    .workflow-step.user .workflow-action{
        background:#eef6ff;color:#1d4ed8;border-top-left-radius:4px
    }
    .workflow-step.agent .workflow-action{
        background:#f2f4f7;color:#344054;border-top-right-radius:4px
    }
    .workflow-output-label{
        color:#6b7280;font-size:.68rem;font-weight:800;margin:.48rem 0 .16rem;
        text-align:right
    }
    .chat-turn{
        display:flex;flex-direction:column;margin:0 0 .55rem
    }
    .chat-turn.user{align-items:flex-start}
    .chat-turn.assistant{align-items:flex-end}
    .chat-label{
        color:#6b7280;font-size:.68rem;font-weight:700;margin:0 0 .16rem
    }
    .chat-bubble{
        max-width:92%;border-radius:12px;padding:.48rem .62rem;
        font-size:.82rem;line-height:1.42;white-space:pre-wrap;word-break:break-word;
        box-sizing:border-box
    }
    .chat-turn.user .chat-bubble{
        background:#0071e3;color:#ffffff;border-top-left-radius:4px
    }
    .chat-turn.assistant .chat-bubble{
        background:#f2f4f7;color:#1d1d1f;border-top-right-radius:4px
    }
    </style>
...
def workflow_actor(agent: str) -> str:
    """이벤트 agent 값을 화면용 주체 이름으로 변환합니다."""

    names = {
        "user": "사용자",
        "super_agent": "Super Agent",
        "pi_super_agent": "Super Agent",
        "select_agent": "select_agent",
        "recommend_agent": "recommend_agent",
        "validate_agent": "validate_agent",
        "change_agent": "change_agent",
        "finalize_agent": "finalize_agent",
        "human_in_the_loop": "사용자 확인",
        "pimono_server": "Pi 서버",
    }
    return names.get(agent, agent)


def workflow_action(item: AgentEvent) -> str:
    """이벤트를 사용자가 읽기 쉬운 행동 설명으로 변환합니다."""

    if item.type == "turn_start":
        return "업무 처리를 시작했습니다."
    if item.type == "message_start":
        return "입력을 접수했습니다."
    if item.type == "message_end":
        return "입력 전달을 완료했습니다."
    if item.type == "routing_end":
        intent = item.payload.get("json", {}).get("intent") if isinstance(item.payload.get("json"), dict) else None
        return f"요청 의도를 판단했습니다{f' ({intent})' if intent else ''}."
    if item.type == "tool_execution_end":
        return item.message or "도구 실행을 완료했습니다."
    if item.type == "turn_end":
        return "업무 처리를 종료했습니다."
    if item.type == "agent_end":
        return "최종 응답을 준비했습니다."
    return item.message or item.type


def append_workflow_history(prompt: str, answer: str, events: list[AgentEvent]) -> None:
    """한 번의 실행 흐름을 업무 진행 이력에 누적합니다."""

    steps: list[dict[str, str]] = [{"actor": "사용자", "action": f"'{prompt}' 요청을 입력했습니다."}]
    for item in events:
        if item.type == "agent_start":
            continue
        if item.type == "message_start" and item.agent == "user":
            continue
        action = workflow_action(item)
        if not action:
            continue
        steps.append({"actor": workflow_actor(item.agent), "action": action})
    history = cast(list[dict[str, Any]], st.session_state["workflow_history"])
    history.append({"steps": steps, "output": answer})
    st.session_state["workflow_history"] = history[-12:]


def workflow_step_side(actor: str) -> str:
    """업무 진행 이력 말풍선의 좌우 위치를 결정합니다."""

    return "user" if actor == "사용자" else "agent"
...
def execute_prompt(runtime: Runtime, prompt: str) -> AgentTurnState:
    ...
    append_workflow_history(prompt, result.final_answer, stream.events)
...
def render_enter_submit_script() -> None:
    """사용자 입력란에서 Enter 입력 시 기존 조회/문의 버튼 로직을 실행합니다."""
    ...
    components.html(
        ...
        const inputGuide = "\\uc544\\ub798\\uc640 \\uac19\\uc774 \\uc0ac\\uc6a9\\ud558\\uc138\\uc694.";
        const enterSubmitLabel = "Enter \\uc2e4\\ud589";

        const findInput = () => Array.from(doc.querySelectorAll("textarea")).find((node) => {
            const placeholder = node.getAttribute("placeholder") || "";
            return placeholder.includes(inputGuide);
        }) || doc.querySelector("textarea");

        const findEnterSubmitButton = () => Array.from(doc.querySelectorAll("button")).find((button) =>
            (button.innerText || "").trim() === enterSubmitLabel
        );

        const hideEnterSubmitButton = () => {
            const enterSubmitButton = findEnterSubmitButton();
            if (enterSubmitButton) {
                const wrapper = enterSubmitButton.closest("div[data-testid='stButton']");
                if (wrapper) {
            ...
            return Boolean(enterSubmitButton);
        };

        const submitFromEnter = (textarea) => {
            textarea.dispatchEvent(new Event("input", { bubbles: true }));
            textarea.dispatchEvent(new Event("change", { bubbles: true }));
            textarea.blur();
            window.setTimeout(() => {
                const enterSubmitButton = findEnterSubmitButton();
                if (enterSubmitButton && !enterSubmitButton.disabled) {
                    enterSubmitButton.click();
                    enterSubmitButton.dispatchEvent(new MouseEvent("click", {
                        bubbles: true,
                        cancelable: true,
                        view: window
                    }));
        ...
        const bindEnterSubmit = () => {
            const textarea = findInput();
            hideEnterSubmitButton();
            if (!textarea || doc.body.dataset.dashEnterSubmitBound) {
                return Boolean(textarea);
            }

            doc.body.dataset.dashEnterSubmitBound = "true";
            const handler = (event) => {
                if (
                    event.key !== "Enter" ||
                    event.shiftKey ||
            ...
            doc.addEventListener("keydown", (event) => {
                ...
                if (placeholder && !placeholder.includes(inputGuide)) {
                    return;
                }
                event.preventDefault();
                event.stopPropagation();
                submitFromEnter(active);
            }, true);
            return true;
        };
        ...
        const timer = window.setInterval(() => {
            attempts += 1;
            if (bindEnterSubmit() || attempts > 20) {
...
def render_chat_history() -> None:
    """주체별 행동 흐름과 최종 Agent output을 업무 진행 이력으로 표시합니다."""

    history = cast(list[dict[str, Any]], st.session_state["workflow_history"])
    if not history:
        body = '<div class="chat-empty">업무가 진행되면 주체별 처리 단계와 Agent output이 여기에 표시됩니다.</div>'
    else:
        turns: list[str] = []
        for turn in history:
            step_rows: list[str] = []
            for step in turn.get("steps", []):
                actor = html.escape(str(step.get("actor", "")))
                action = html.escape(str(step.get("action", "")))
                side = workflow_step_side(str(step.get("actor", "")))
                step_rows.append(
                    f'<div class="workflow-step {side}">'
                    f'<div class="chat-label">{actor}</div>'
                    f'<div class="workflow-action">{action}</div>'
                    '</div>'
                )
            output = html.escape(str(turn.get("output", "")).strip() or "응답 내용이 없습니다.")
            turns.append(
                '<div class="workflow-turn">'
                f'{"".join(step_rows)}'
                '<div class="workflow-output-label">Agent output</div>'
                '<div class="chat-turn assistant">'
                f'<div class="chat-bubble">{output}</div>'
                '</div>'
                '</div>'
            )
        body = "".join(turns)

    st.markdown('<div class="panel-title">[ 업무 진행 이력 ]</div>', unsafe_allow_html=True)
    st.markdown(f'<div id="chat-history-scroll" class="chat-panel">{body}</div>', unsafe_allow_html=True)
    if components is not None:
        components.html(
            """
            <script>
            const chat = window.parent.document.getElementById("chat-history-scroll");
            if (chat) {
...
def render_app() -> None:
    ...
    header_area, _ = st.columns([3.8, 1.21], gap="large")
    with header_area:
        header_content, _ = st.columns([2, 1], gap="small")
        with header_content:
            st.markdown(
                f"""
    ...
    work_area, chat_area, log_area = st.columns([2.533, 1.267, 1.21], gap="large")
    with work_area:
        st.markdown('<div class="panel-title">[ 사용자 입력 ]</div>', unsafe_allow_html=True)
        user_text = st.text_area(
            "입력",
            height=150,
            placeholder="아래와 같이 사용하세요.\nex 1) 작업이 시작된 일시 (용어 조회/추천을 위한 설명)\nex 2) 표준용어 등록 절차가 뭐야? (문의)\nex 3) '작업시작일시'의 설명을 '작업이 시작된 일시'로 수정 (변경)\nex 4) '작업시작일시' 삭제 (변경)",
            label_visibility="collapsed",
        )
        if st.button("Enter 실행", key="enter_submit_action") and user_text.strip():
            set_change_result_alert(False)
            result = execute_prompt(runtime, user_text.strip())
            if result.awaiting_human_confirmation:
                st.rerun()
        lookup_col, recommend_col = st.columns([1, 1], gap="small")
        with lookup_col:
        ...
        with recommend_col:
            ...
            if st.button("추천", disabled=recommend_disabled, use_container_width=True):
                ...
                st.session_state["last_qna_question"] = previous_last_qna_question
                st.session_state["last_description"] = previous_last_description
        render_enter_submit_script()
        lookup_result_col, recommend_result_col = st.columns([1, 1], gap="small")
