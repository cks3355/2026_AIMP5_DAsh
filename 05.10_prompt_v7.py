...
# DAsh: Pi-mono Super Agent routing + LangGraph Sub Agent prototype v7.
#
...
# - v7은 표준용어 설명 검색 Vector DB와 분리된 문의답변 RAG Vector DB를 두고, 앙상블 검색과 재랭킹 후 LLM 답변을 생성합니다.
#
# 실행 방법:
#     streamlit run 05.10_prompt_v7.py
#     python 05.10_prompt_v7.py "표준용어 등록 절차가 뭐야?"
#     python 05.10_prompt_v7.py --docs
...
class DashGraph:
    ...
    def _build_graph(self) -> Any:
        ...
        graph.add_conditional_edges(START, self._route, {"select_agent": "select_agent", "recommend_agent": "recommend_agent", "validate_agent": "validate_agent", "change_agent": "change_agent", "finalize_agent": "finalize_agent"})
    ...
    def validate_agent(self, state: AgentTurnState) -> AgentTurnState:
        ...
        try:
            ...
            if state.validation.ok:
                ...
                state.logs.append(event("tool_execution_end", "human_in_the_loop", "사용자 최종 의도 확인 대기", request=req.model_dump()))
        except Exception as exc:
...
class ProjectDocuments:
    ...
    def _built_in_faq(self) -> list[AnswerDocument]:
        ...
        faqs = [
            ...
            (
                "문의 답변 RAG 처리 절차",
                "v7은 표준용어 설명 검색 Vector DB와 별개로 문의답변 Vector DB를 구성합니다. 사용자 문의는 vector score와 keyword score 앙상블으로 후보를 찾고 재랭킹한 뒤 LLM이 근거 기반 답변을 생성합니다.",
                "faq",
                ["RAG", "문의", "앙상블", "재랭킹", "LLM"],
            ),
        ]
...
def build_runtime() -> Runtime:
    ...
    return Runtime(config, repo, vector, answer_vector, PiMonoSuperAgent(DashGraph(tools), llm), documents)
...
def execute_prompt(runtime: Runtime, prompt: str) -> AgentTurnState:
    ...
    if result.router_decision and result.router_decision.intent == Intent.DA_REQUEST:
        st.session_state["qa_answer"] = result.final_answer
...
def render_qa_result() -> None:
    """문의 답변 RAG 결과와 근거를 표시합니다."""

    st.markdown('<div class="panel-title">문의 답변</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div class="result-box">{st.session_state["qa_answer"] or "문의 버튼을 누르면 RAG 기반 답변이 표시됩니다."}</div>',
        unsafe_allow_html=True,
    )
...
def render_app() -> None:
    ...
    st.markdown('<div class="dash-lead">DAsh는 표준용어 관리를 돕는 Pi-mono Super Agent입니다. 사용자 설명은 표준용어 Vector DB에서 조회하고, 문의사항은 별도 문의답변 Vector DB를 RAG로 조회해 앙상블/재랭킹 후 LLM 답변을 생성합니다.</div>', unsafe_allow_html=True)
    ...
    with work_area:
        ...
        lookup_col, recommend_col, qa_col, change_col = st.columns([1, 1, 1, 1], gap="small")
        with lookup_col:
        ...
        with recommend_col:
            ...
            if st.button("추천", disabled=recommend_disabled, use_container_width=True):
                ...
                st.session_state["answer"] = result.final_answer
        with qa_col:
            if st.button("문의", use_container_width=True) and user_text.strip():
                result = execute_prompt(runtime, "문의: " + user_text.strip())
                st.session_state["qa_answer"] = result.final_answer
        with change_col:
        ...
        lookup_result_col, recommend_result_col, qa_result_col, change_result_col = st.columns([1, 1, 1, 1], gap="small")
...
def run_cli(args: list[str]) -> int:
    ...
    if "--docs" in args:
        ...
        print("\n실행 방법: streamlit run 05.10_prompt_v7.py")
        return 0
